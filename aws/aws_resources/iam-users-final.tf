...
#IAM group to gain permissions to administer openidl specific aws resources
resource "aws_iam_policy" "openidl_admin_policy" {
  name = "${local.std_name}-OpenidlAdmPolicy"
  policy = jsonencode({
    "Version": "2012-10-17",
    "Statement": [
        {
            ...
            "Resource": "*",
            "Condition": {
                "StringEquals": {"aws:ResourceTag/owner": "arn:aws:iam::${var.aws_account_number}:role/${local.std_name}-openidl-admin"}
            }
        },
    ]
  })
...
resource "aws_iam_role" "openidl_admin_role" {
  name = "${local.std_name}-openidl-admin"
  assume_role_policy = jsonencode({
  "Version": "2012-10-17",
  "Statement": [
    {
  ...
  managed_policy_arns = [aws_iam_policy.openidl_admin_policy.arn]
  tags = merge(local.tags, {name = "${local.std_name}-openidl-admin", cluster_type = "both"})
  description = "The iam role that is used to manage openidl related AWS resources"
  max_session_duration = 3600
}
resource "aws_iam_group" "openidl_admin_group" {
  name = "${local.std_name}-openidl-admin"
}
resource "aws_iam_group_policy_attachment" "openidl_admin_group_policy_attachment" {
  group = aws_iam_group.openidl_admin_group.name
  policy_arn = aws_iam_policy.openidl_admin_group_assume_policy.arn
}
resource "aws_iam_policy" "openidl_admin_group_assume_policy" {
  name = "${local.std_name}-OpenidlAdmAssumePolicy"
  policy = jsonencode({
    "Version": "2012-10-17",
    "Statement": [
      {
        ...
        "Effect": "Allow",
        "Action": "sts:AssumeRole",
        "Resource": aws_iam_role.openidl_admin_role.arn
      }]
  })
  tags = merge(local.tags, {
    name = "${local.std_name}-OpenidlAdmAssumePolicy",
