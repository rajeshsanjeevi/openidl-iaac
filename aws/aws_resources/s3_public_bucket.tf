...
resource "aws_s3_bucket" "s3_bucket_logos_public" {
  bucket = "${local.std_name}-${var.s3_bucket_name_logos_public}"
  acl    = "public-read"
  force_destroy = true
  versioning {
    enabled = true
  ...
  tags = merge(
    local.tags,
    {
      "Name" = "${local.std_name}-${var.s3_bucket_name_logos_public}"
    },)
}
  ...
  policy = jsonencode({
    ...
    "Statement": [
        ...
        },
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource = "arn:aws:s3:::${local.std_name}-${var.s3_bucket_name_logos_public}/*",
        Condition = {
          Bool = {
